...
const INFILL_PATTERNS = ["grid", "gyroid", "cubic", "rectilinear", "honeycomb"];
const ADHESION_MODES = ["none", "skirt", "brim"];
...
export function normalizePrintSettings(raw) {
    ...
    const speeds = (input.speeds && typeof input.speeds === "object" ? input.speeds : {});
    return {
        ...
        infillPattern: INFILL_PATTERNS.includes(input.infillPattern)
            ? input.infillPattern
        ...
        adhesion: ADHESION_MODES.includes(input.adhesion)
            ? input.adhesion
        ...
        firstLayerNozzleTemp: clamp(finiteNumber(input.firstLayerNozzleTemp, DEFAULT_PRINT_SETTINGS.firstLayerNozzleTemp), 150, K2_SE_PROFILE.maxNozzleTemp),
        bedTemp: clamp(finiteNumber(input.bedTemp, DEFAULT_PRINT_SETTINGS.bedTemp), 0, K2_SE_PROFILE.maxBedTemp),
        firstLayerBedTemp: clamp(finiteNumber(input.firstLayerBedTemp, DEFAULT_PRINT_SETTINGS.firstLayerBedTemp), 0, K2_SE_PROFILE.maxBedTemp),
...
export function validatePrintSettings(settings) {
    const errors = [];
